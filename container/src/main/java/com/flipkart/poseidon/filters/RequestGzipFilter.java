...
/**
 * Filter to decompress request body for POST & PUT requests only if content-encoding header has gzip
 *
 * Created by mohan.pandian on 13/05/16.
 */
public class RequestGzipFilter implements Filter {
    ...
    public void doFilter(final ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        boolean requestTypeSupported = HttpMethod.POST.equals(servletRequest.getMethod()) ||
                HttpMethod.PUT.equals(servletRequest.getMethod());
        if (isGzipped && !requestTypeSupported) {
            throw new IllegalStateException(new StringBuilder()
                    ...
                    .append(" Only POST & PUT requests are currently supported.")
                    .toString()
            );
        }
        if (isGzipped) {
